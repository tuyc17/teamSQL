...
public class IServiceHandler implements IService.Iface {
    ...
    // ?
//    public ExecuteStatementResp statementHandler(String statement) {
//        Table tempTable,table;
//        boolean success;
//        int retCase;
//        // long session = req.sessionId;
//        // 转成小写以规避大小写问题
//        CharStream input = CharStreams.fromString(statement.toLowerCase());
//        // 特殊input的判断与分析
//        ExecuteStatementResp resp = new ExecuteStatementResp();
//        resp.setStatus(new Status(Global.SUCCESS_CODE));
//
//        if(input.toString().equals("set auto commit true")) {
//            Global.AUTO_COMMIT = true;
//            Global.TRANSACTION = false;
//            resp.status.msg ="设置为auto commit模式";
//            return resp;
//        }
//        else if(input.toString().equals("set auto commit false")) {
//            Global.AUTO_COMMIT = false;
//            resp.status.msg = "已取消auto commit";
//            return resp;
//        }
//        else if(input.toString().equals("begin transaction")) {
//            if(Global.AUTO_COMMIT) {
//                resp.status.code = Global.FAILURE_CODE;
//                resp.status.msg = "无效的操作：已处于auto commit模式";
//            }
//            else if(Global.TRANSACTION) {
//                resp.status.code = Global.FAILURE_CODE;
//                resp.status.msg = "无效的操作：transaction已经启动";
//            }
//            else {
//                resp.status.msg = "启动transaction。（输入操作指令完成后，请手动输入commit指令）";
//                Global.TRANSACTION = true;
//                // 将当前数据库的表存入save/commit0下
//                pasteFolder("/data", "/save/commit"+ Global.COMMIT_VERSION);
//                Global.COMMIT_VERSION += 1;
//            }
//            return resp;
//        }
...
//            }
//            else if(Global.COMMIT_VERSION == 0) {
//                resp.status.code = Global.FAILURE_CODE;
//                resp.status.msg = "已经是最初始的版本了";
//            }
//            else {
//                // 将最近的一次commit：save/commitX中的数据回滚
//                pasteFolder("/save/commit"+ Global.COMMIT_VERSION, "/data");
//                clearFolder("/save/commit"+ Global.COMMIT_VERSION);
//                Global.COMMIT_VERSION -= 1;
//                resp.status.msg = "成功回滚至第"+Global.COMMIT_VERSION+"次commit的版本";
//            }
//            return resp;
//        }
...
//            if(Global.AUTO_COMMIT) {
//                resp.status.code = Global.FAILURE_CODE;
//                resp.status.msg ="已处于auto commit模式，无需手动commit";
//            }
//            else if(!Global.TRANSACTION) {
//                resp.status.code = Global.FAILURE_CODE;
//                resp.status.msg ="无效的指令：尚未处在一个事务中";
...
//            else {
//                try {
//                    // TODO:读取transaction_commands.txt中的指令，逐个执行
//
//                    // TODO:清空transaction_commands.txt
//                } catch(Exception e) {
//                    // TODO:做rollback该做的事
//
//                } finally {
